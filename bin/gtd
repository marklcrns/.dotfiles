...
# ---------------------------------------------------------------
# Updates:
# 2020-06-12: Added WSL support. gtd .wav files needs to be stored at
...
then
  ...
  SPEAK_CMD="${SPEAK_CMD:-"cd /mnt/c/Users/${WIN_USERNAME}/Documents/gtd/espeak; cmd.exe /c start"}"
else
  # TODO: Fix this
  SPEAK_CMD="${SPEAK_CMD:-"&>/dev/null aplay"}"
fi
...
SPEAK_WORK="${SPEAK_WORK:-"./espeak_gtd_work_m_v4.wav"}"
SPEAK_BREAK="${SPEAK_BREAK:-"./espeak_gtd_break_f_v4.wav"}"
...
do_options() {
   ...
   if [[ -n "$DO_MPD" ]]; then
      if [[ -n "$in_break" ]]; then
          sleep 1 && eval "$MPD_CMD_BREAK"
      else
          sleep 1 && eval "$MPD_CMD_WORK"
      fi
   fi
